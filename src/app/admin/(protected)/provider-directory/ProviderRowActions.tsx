...
import { setProviderStatusAction, deleteProviderAction } from "./actions";

export function ProviderRowActions({ id, status }: { id: number; status: string }) {
  const [isPending, setIsPending] = useState(false);

  async function setStatus(next: "approved" | "rejected") {
    setIsPending(true);
    const result = await setProviderStatusAction(id, next);
    setIsPending(false);
    if (!result.success) toast.error(result.error);
    else toast.success(next === "approved" ? "Approved — now live on /provider-directory." : "Rejected.");
  }
